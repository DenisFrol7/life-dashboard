...
export function SeriesDetailsPage() {
  ...
  return <div className="series-details-page">
    ...
    <section className="series-hero-card">
      ...
      <div className="movie-personal-info"><small>Мой статус</small><strong>{library ? statusLabels[library.status] : 'Не в библиотеке'}</strong><small>Моя оценка</small><strong>{library?.rating ? `★ ${library.rating}/10` : '—'}</strong><small>Заметки</small><p>{library?.personalNote ?? 'Заметок пока нет.'}</p></div>
    </section>
...
function SeasonForm({ series, season, onClose, onSaved }: { series: Series; season?: Season; onClose: () => void; onSaved: () => void }) {
...
function EpisodeForm({ season, episode, onClose, onSaved }: { season: Season; episode?: Episode; onClose: () => void; onSaved: () => void }) {
...
function BulkEpisodeForm({ season, onClose, onSaved }: { season: Season; onClose: () => void; onSaved: () => void }) {
...
function SeasonCompletionForm({ season, onClose, onSaved }: { season: Season; onClose: () => void; onSaved: () => void }) {
