...
export function SeriesDetailsPage() {
  ...
  return <div className="series-details-page">
    ...
    <section className="series-hero-card">
      ...
      <div className="movie-main-info"><p className="eyebrow">{series.format === 'ANIMATION' ? 'Мультсериал' : 'Сериал'}</p><h1>{series.title}</h1><h2 className="original-title-slot" aria-hidden={!series.originalTitle}>{series.originalTitle || '\u00a0'}</h2><dl><div><dt>Год</dt><dd>{series.releaseYear ?? '—'}</dd></div><div><dt>Жанр</dt><dd>{series.genre ?? '—'}</dd></div><div><dt>Статус выпуска</dt><dd>{releaseLabels[series.releaseStatus]}</dd></div></dl></div>
...
export function EpisodeForm({ season, episode, onClose, onSaved }: { season: Season; episode?: Episode & { watches?: Watch[] }; onClose: () => void; onSaved: () => void }) {
  const latestWatch = episode?.watches?.at(-1)
  const [number, setNumber] = useState(episode?.episodeNumber ?? 1); const [title, setTitle] = useState(episode?.title ?? ''); const [minutes, setMinutes] = useState<number | null>(episode?.durationMinutes ?? null); const [date, setDate] = useState<string | null>(episode?.releaseDate ?? null); const [watchedAt, setWatchedAt] = useState(latestWatch ? new Intl.DateTimeFormat('sv-SE').format(new Date(latestWatch.watchedAt)) : ''); const [error, setError] = useState<string | null>(null)
  const submit = async (event: FormEvent) => { event.preventDefault(); try { const input = { episodeNumber: number, title, durationMinutes: minutes, releaseDate: date }; const saved = episode ? await updateEpisode(episode.id, input) : await createEpisode(season.id, input); if (watchedAt) { const instant = new Date(`${watchedAt}T12:00:00`).toISOString(); if (latestWatch) await updateEpisodeWatch(latestWatch.id, instant); else await watchEpisode(saved.id, instant) } onSaved() } catch (reason) { setError(reason instanceof Error ? reason.message : 'Не удалось сохранить эпизод') } }
