...
import { createKinopoiskSeries, createSeries, deleteSeries, getSeriesCatalog, previewKinopoiskSeries, putSeriesLibrary, searchKinopoiskSeries, updateSeries, type KinopoiskSeriesCandidate, type Series, type SeriesFormat, type SeriesInput, type SeriesLibraryEntry } from '../api/series'
...
export function SeriesForm({ series, library, onClose, onSaved, onDeleted }: { series?: Series; library?: SeriesLibraryEntry; onClose: () => void; onSaved: () => void; onDeleted?: () => void }) {
  const { showToast } = useToast()
  ...
  const [candidates, setCandidates] = useState<KinopoiskSeriesCandidate[]>([])
  const [selectedFilmId, setSelectedFilmId] = useState<number | null>(null)
  const [catalogSummary, setCatalogSummary] = useState<{ seasons: number; episodes: number } | null>(null)
  ...
  const searchCatalog = async () => { if (searchQuery.trim().length < 2) return; setSearching(true); setError(null); try { setCandidates(await searchKinopoiskSeries(searchQuery.trim())) } catch (reason) { setError(getApiErrorMessage(reason, 'Не удалось выполнить поиск в Кинопоиске')) } finally { setSearching(false) } }
  const selectCandidate = async (candidate: KinopoiskSeriesCandidate) => { if (candidate.existingContentId) return; setSearching(true); setError(null); try { const details = await previewKinopoiskSeries(candidate.filmId); setSelectedFilmId(details.filmId); setCatalogSummary({ seasons: details.seasonCount, episodes: details.episodeCount }); setItem({ ...emptySeries, title: details.title, originalTitle: details.originalTitle, releaseYear: details.releaseYear, description: details.description, coverUrl: details.coverUrl, releaseStatus: details.releaseStatus, genre: details.genre }); setCandidates([]) } catch (reason) { setError(getApiErrorMessage(reason, 'Не удалось загрузить данные сериала')) } finally { setSearching(false) } }
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(null); try { const saved = series ? await updateSeries(series.id, item) : selectedFilmId ? await createKinopoiskSeries(selectedFilmId, item) : await createSeries(item); if (inLibrary) await putSeriesLibrary(saved.id, entry); showToast(series ? 'Сериал обновлён' : 'Сериал добавлен'); onSaved() } catch (reason) { setError(getApiErrorMessage(reason, 'Не удалось сохранить сериал')) } finally { setSaving(false) } }
  const remove = async () => { if (!series || !window.confirm(`Удалить «${series.title}» вместе с сезонами и историей?`)) return; try { await deleteSeries(series.id); showToast('Сериал удалён'); (onDeleted ?? onSaved)() } catch (reason) { setError(reason instanceof Error ? reason.message : 'Не удалось удалить сериал') } }
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><form className="habit-form movie-form" onSubmit={(event) => void submit(event)}><div className="form-heading"><div><p className="eyebrow">Сериалы</p><h2>{series ? 'Редактирование' : 'Новый сериал'}</h2></div><button type="button" onClick={onClose}>×</button></div>{error && <div className="form-error">{error}</div>}{!series && <section className="kinopoisk-movie-search"><div><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void searchCatalog() } }} placeholder="Введите название сериала" /><button className="secondary-button" type="button" disabled={searching || searchQuery.trim().length < 2} onClick={() => void searchCatalog()}>{searching ? 'Ищем…' : 'Найти в Кинопоиске'}</button></div>{selectedFilmId && <p>Данные и структура загружены: {catalogSummary?.seasons ?? 0} сезонов, {catalogSummary?.episodes ?? 0} эпизодов. Проверьте поля и укажите личный статус.</p>}{candidates.length > 0 && <div className="kinopoisk-movie-results">{candidates.map((candidate) => <button type="button" key={candidate.filmId} disabled={Boolean(candidate.existingContentId)} onClick={() => void selectCandidate(candidate)}><span>▤</span><span><strong>{candidate.nameRu || candidate.nameOriginal || `Сериал ${candidate.filmId}`}</strong><small>{candidate.nameOriginal && candidate.nameOriginal !== candidate.nameRu ? `${candidate.nameOriginal} · ` : ''}{candidate.year ?? 'Год неизвестен'}{candidate.existingContentId ? ' · Уже в каталоге' : ''}</small></span></button>)}</div>}</section>}<label>Название<input required value={item.title} onChange={(event) => setItemValue('title', event.target.value)} /></label><label>Оригинальное название<input value={item.originalTitle ?? ''} onChange={(event) => setItemValue('originalTitle', event.target.value || null)} /></label><div className="form-grid"><label>Формат<select value={item.format} onChange={(event) => setItemValue('format', event.target.value as SeriesFormat)}><option value="LIVE_ACTION">Сериал</option><option value="ANIMATION">Мультсериал</option></select></label><label>Статус выпуска<select value={item.releaseStatus} onChange={(event) => setItemValue('releaseStatus', event.target.value as ReleaseStatus)}>{Object.entries(releaseLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label>Год<input min="1900" type="number" value={item.releaseYear ?? ''} onChange={(event) => setItemValue('releaseYear', event.target.value ? Number(event.target.value) : null)} /></label><label>Жанр<input maxLength={100} value={item.genre ?? ''} onChange={(event) => setItemValue('genre', event.target.value || null)} /></label><label>Обложка<input type="url" value={item.coverUrl ?? ''} onChange={(event) => setItemValue('coverUrl', event.target.value || null)} /></label></div><label>Описание<textarea rows={3} value={item.description ?? ''} onChange={(event) => setItemValue('description', event.target.value || null)} /></label><fieldset className="library-fields"><legend>Моя библиотека</legend><label className="all-day-check"><input type="checkbox" checked={inLibrary} onChange={(event) => setInLibrary(event.target.checked)} />Добавить в библиотеку</label>{inLibrary && <><div className="form-grid"><label>Статус<select value={entry.status} onChange={(event) => setEntryValue('status', event.target.value as LibraryStatus)}>{Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label>Оценка<select value={entry.rating ?? ''} onChange={(event) => setEntryValue('rating', event.target.value ? Number(event.target.value) : null)}><option value="">Без оценки</option>{Array.from({ length: 10 }, (_, index) => index + 1).map((value) => <option key={value}>{value}</option>)}</select></label></div><label className="favorite-check"><input type="checkbox" checked={entry.favorite} onChange={(event) => setEntryValue('favorite', event.target.checked)} />В избранном</label><label>Заметка<textarea rows={2} value={entry.personalNote ?? ''} onChange={(event) => setEntryValue('personalNote', event.target.value || null)} /></label></>}</fieldset><div className="form-buttons">{series && <button className="danger-button" type="button" onClick={() => void remove()}>Удалить</button>}<button className="secondary-button" type="button" onClick={onClose}>Отмена</button><button className="primary-button" disabled={saving || searching}>{saving ? 'Сохраняем…' : 'Сохранить'}</button></div></form></div>
}
