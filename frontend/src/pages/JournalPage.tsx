...
export function BlogPage() {
  ...
  const remove = async (entry: JournalEntry) => { if (!window.confirm(`Удалить запись «${entry.title ?? formatDate(entry.entryDate)}»?`)) return; try { await deleteJournalEntry(entry.id); showToast('Запись блога удалена'); setSelected(null); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : 'Не удалось удалить запись') } }

  return <div className="journal-page">
    <section className="journal-toolbar"><div className="journal-search"><span><Search /></span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск по записям" /></div><button className="primary-button icon-button" onClick={() => setEditing('new')}><Plus />Новая запись</button></section>
    ...
    <section className="journal-layout">
      <aside className="journal-list"><div className="journal-list-heading"><span>{visible.length} записей</span></div>{loading ? <div className="loading"><span /></div> : visible.length === 0 ? <div className="journal-empty">Записей не найдено.</div> : visible.map((entry) => <button className={selected?.id === entry.id ? 'active' : ''} key={entry.id} onClick={() => setSelected(entry)}><div><time>{formatDate(entry.entryDate)}</time>{entry.pinned && <span title="Закреплено"><Pin /></span>}</div><strong>{entry.title ?? 'Без заголовка'}</strong><p>{excerpt(entry.content, 95)}</p><div className="entry-tags">{entry.tags.slice(0, 3).map((item) => <i key={item.id}>{item.name}</i>)}</div></button>)}</aside>
      <article className="journal-reader">{selected ? <><header><div><p className="eyebrow">{formatDate(selected.entryDate)}</p><h2>{selected.title ?? 'Без заголовка'}</h2></div><div><button className="icon-button" onClick={() => setEditing(selected)}><Pencil />Редактировать</button><button className="icon-button" onClick={() => void remove(selected)}><Trash2 />Удалить</button></div></header>{selected.tags.length > 0 && <div className="reader-tags">{selected.tags.map((item) => <span key={item.id}>#{item.name}</span>)}</div>}<div className="journal-content">{selected.content.split('\n').map((paragraph, index) => <p key={index}>{paragraph || <br />}</p>)}</div></> : <div className="reader-empty"><span><NotebookPen /></span><h2>Ваш блог</h2><p>Выберите запись слева или создайте новую.</p></div>}</article>
    </section>
