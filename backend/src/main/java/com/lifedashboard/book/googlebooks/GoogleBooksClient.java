...
@Component public class GoogleBooksClient{
 ...
 public List<BookData> search(String query){
  ...
  }catch(RuntimeException e){if(e instanceof InvalidRequestException invalid)throw invalid;log.warn("Google Books search failed: {}",e.toString());throw new InvalidRequestException("Не удалось подключиться к API Google Books");}
 }
 ...
 private InvalidRequestException apiError(RestClientResponseException e){if(e.getStatusCode().value()==429&&apiKey.isBlank())return new InvalidRequestException("Суточный лимит Google Books исчерпан. Добавьте GOOGLE_BOOKS_API_KEY в файл .env");return new InvalidRequestException("Запрос к API Google Books завершился ошибкой "+e.getStatusCode().value());}
